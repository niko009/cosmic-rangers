class Game{
  ...
  bossDefeated(){
    ...
    this.aggression = Math.min(2.2, 1 + this.bossesKilled * 0.13);
    ...
    for(const m of this.meteors){
      ...
      for(const s of ships){
        if(Math.hypot(m.x-s.x,m.y-s.y)<m.r+s.r){
          m.dead=true;
          const dmg = m.big ? 25 : (m.kind ? 20 : 16);
          this.damage(dmg);
  ...
  onEnemyLeaked(m){
    const dmg = m.big ? 20 : (m.kind === "drone" ? 14 : m.kind ? 12 : 10);
    this.damage(dmg);
